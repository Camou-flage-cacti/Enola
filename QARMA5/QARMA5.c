...
int main() {
	text_t plaintext = 0x11000D4F;
	key_t w0 = 0x3344112233441122;
	key_t k0 = 0x3344112233441122;
	tweak_t tweak = 0x0;
	text_t ciphertext; //0xBEE4599A

	printf("QARMA64  Use Sbox %d\nPlaintext = 0x%016llx\nKey = 0x%016llx || 0x%016llx\nTweak = 0x%016llx\n\n", sbox_use, plaintext, w0, k0, tweak);


	ciphertext = qarma64_enc(plaintext, tweak, w0, k0, 1);
	printf("Round 1: \nCiphertext = 0x%016llx  %s\n", ciphertext, (ciphertext == check_box[0]) ? "√" : "×");
	ciphertext = qarma64_dec(ciphertext, tweak, w0, k0, 1);
	printf("Plaintext  = 0x%016llx  %s\n", ciphertext, (ciphertext == plaintext) ? "√" : "×");

	ciphertext = qarma64_enc(plaintext, tweak, w0, k0, 3);
	printf("Round 3: \nCiphertext = 0x%016llx  %s\n", ciphertext, (ciphertext == check_box[0]) ? "√" : "×");
	ciphertext = qarma64_dec(ciphertext, tweak, w0, k0, 3);
	printf("Plaintext  = 0x%016llx  %s\n", ciphertext, (ciphertext == plaintext) ? "√" : "×");
