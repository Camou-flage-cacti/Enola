...
void ARMFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  Function &EnolaF = MF.getFunction();
  if(EnolaF.hasMetadata("Enola-non-leaf"))
  {
    outs()<<"ARMframelowering "<< EnolaF.getName().str()<<" has Enola-non-leaf metadata\n";
    if(!SavedRegs.test(ARM::LR))
    {
      outs()<<"Enola ARMframelowering: LR not present in callee saved  register list\n";
      SavedRegs.set(ARM::LR);
    }
    else{
      outs()<<"Enola ARMframelowering: LR already present in callee saved  register list\n";
